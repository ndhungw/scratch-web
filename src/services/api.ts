...
export const updateUser = (id: string, dataToUpdate: any) => {
  const found = users_table.filter((item) => item.id === id)[0];
...
export const getKitchenSector = (idUser: string, isInSaved: boolean) => {
  const promise = new Promise((resolve) => {
...
export const getFollowingSector = (idUser: string) => {
  const promise = new Promise((resolve) => {
    // fake temp
    const ret = {
      name: "following",
...
export const getKitchenData = (idUser: string) => {
  const promise = new Promise((resolve) => {
    // recipes sector
    const cookbooksOfRecipesSector = getCookbooks(idUser, false);
...
export const getRecipe = (idRecipe: string) => {
  const found = recipes_table.filter((recipe) => recipe.id === idRecipe)[0];
  return found;
};

export const saveRecipe = (idFeed: string, idCookbook: string) => {
  const promise = new Promise((resolve, reject) => {
